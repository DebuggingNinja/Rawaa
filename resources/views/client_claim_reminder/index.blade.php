...
<div class="container-fluid">
        ...
        <div class="breadcrumb-main add-contact justify-content-sm-between ">
          <div class=" d-flex flex-wrap justify-content-center breadcrumb-main__wrapper">
            <div class="d-flex align-items-center add-contact__title justify-content-center me-sm-25">
              <h4 class="text-capitalize fw-500 breadcrumb-title">{{ __('client.Clients\' Claim Reminders') }}
              </h4>
          ...
          <div class="breadcrumb-main__wrapper">
            ...
            <form action="{{route('clients.claims.reminder')}}" method="GET"
    ...
    <div class="col-lg-12 mb-30">
      <div class="card">
        <div class="card-header color-dark fw-500">
          {{trans('client.Scheduled Payments')}}
        </div>
        <div class="card-body">
              ...
              <table class="table mb-0 table-borderless">
                <thead>
                  <tr class="userDatatable-header">
                    ...
                    <th>
                      <span class="userDatatable-title">{{trans('client.payment_date')}}</span>
                    </th>
                    <th>
                      <span class="userDatatable-title">{{trans('client.paymenr_status')}}</span>
                    </th>
                    <th>
                  ...
                  <tr>
                      ...
                      <div class="d-flex">
                        ...
                        <div class="userDatatable-inline-title">
                          <a href="#" class="text-dark fw-500">
                            <h6>{{ $paymentReminder->client?->name??'--' }}</h6>
                          </a>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div class="userDatatable-content">
                        {{ $paymentReminder->client?->code??'--' }}
                      </div>
                    </td>
                    <td>
                      <div class="userDatatable-content">
                        @if(($paymentReminder->client?->balance??0) > 0)
                          <span class="text-danger fw-bold">
                          {{
                          __(
                          'client.We owe the client :amount RMB'
                          ,['amount'=>number_format(($paymentReminder->client?->balance ?? 0)
                          *-1, 2, '.', ',')]) }}
                        </span>
                        @elseif(($paymentReminder->client?->balance??0) == 0)

                        0.00
                        @else
                          <span class="text-success fw-bold">
                          {{ str_replace('-', '', __('client.The Client owe us :amount RMB',[
                              'amount'=>number_format(abs($paymentReminder->client?->balance ?? 0),
                          2, '.', ',')])) }}
                        </span>
                        @endif
                      </div>
                    ...
                    <td>
                      ...
                      @endif
                       d-inline-block w-auto px-2">
                        {{
                        __('client.'.$paymentReminder->status) }}
                      </div>
                    ...
                    <td>
                      <ul class="orderDatatable_actions mb-0 d-flex flex-wrap">
                        <a href="#" class="btn btn-success w-auto mx-2 pay" data-bs-toggle="modal"
                          data-bs-target="#PaymentModal" data-client-id="{{ $paymentReminder->client_id }}"
                          data-payment-amount="{{ abs($paymentReminder->client?->balance??0) }}">{{
                          __('client.pay_now') }}</a>
                        ...
                        <a href="#" class="btn btn-dark w-auto mx-2 change-date" data-bs-toggle="modal"
                          ...
                          __('client.Change Payment Date') }}</a>
                      </ul>
                    </td>
                  </tr>
                  @endforeach
  ...
  <div class="row">
    <div class="col-lg-12 mb-30">
      <div class="card">
        <div class="card-header color-dark fw-500">
          {{trans('client.Unscheduled Payments')}}
        </div>
        <div class="card-body">
          <div class="userDatatable global-shadow border-light-0 w-100">
            <div class="table-responsive">
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="userDatatable-content">
                        ...
                        @else
                          <span class="text-success fw-bold">
                          {{ str_replace('-', '', __('client.The Client owe us :amount RMB',[
                              'amount'=>number_format(abs($client->balance),
                          2, '.', ',')])) }}
                        </span>
                        @endif
                      </div>
                    ...
                    <td>
                      <ul class="orderDatatable_actions mb-0 d-flex flex-wrap">
                        ...
                        <a href="#" class="btn btn-success w-auto mx-2 pay" data-bs-toggle="modal"
                          data-bs-target="#PaymentModal" data-client-id="{{ $client->id }}"
                          data-payment-amount="{{ abs($client->balance??0) }}">{{
                          __('client.pay_now') }}</a>

                        <a href="#" class="btn btn-dark w-auto mx-2 schedule-date" data-bs-toggle="modal"
                          data-bs-target="#schedulePaymentDateModal" data-client-id="{{ $client->id }}">{{
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="changePaymentDateModalLabel">{{ __('client.Change Payment Date') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <!-- Form inside the modal -->
        <form action="{{ route('clients.claims.reminder.change.date') }}" method="POST">
          @csrf
          ...
          <div class="mb-3">
            <label for="new_payment_date" class="form-label">{{ __('client.New Payment Date') }}</label>
...
<div class="modal fade" id="schedulePaymentDateModal" tabindex="-1" aria-labelledby="schedulePaymentDateModalLabel"
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="schedulePaymentDateModalLabel">{{ __('client.Create Payment Date') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <!-- Form inside the modal -->
        <form action="{{ route('clients.claims.reminder.create.date') }}" method="POST">
          @csrf
          <!-- Hidden input for payment_id -->
          <input type="hidden" name="client_id" value="" id="schedulePaymentDateModalClientId">
...
<div class="modal fade" id="PaymentModal" tabindex="-1" aria-labelledby="PaymentModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <!-- Form inside the modal -->
        @include('client_claim_reminder.components.paymentForm')
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  $('.schedule-date').click(function (event) {
    ...
    $('#schedulePaymentDateModalClientId').val(client);
...
<script>
  ...
  $('.pay').click(function (event) {
    ...
    $('#financialModalClientId').val(client);
    $('#amount').val(amount);
