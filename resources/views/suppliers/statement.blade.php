    ...
    <div class="col-lg-12 mb-30">
      <div class="card">
        <div class="card-header color-dark fw-500">
          {{ __('statement.supplier_statement') }}
        </div>
        <div class="card-body">
          <div class="userDatatable global-shadow border-light-0 w-100" id="pdf-content">
            <div class="my-3">
              <table class="table mb-0 table-borderless horizontal-table ">
                <thead>
                  <tr>
                    <th colspan="2">{{ __('statement.supplier_data') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td><strong>{{ __('statement.name') }}</strong> &nbsp; {{ $supplier->name }}</td>
                    <td><strong>{{ __('statement.code') }}</strong> &nbsp; {{ $supplier->code }}</td>
                  </tr>
                  <tr>
              ...
              <form id="form_select_warehouse" action="{{route('suppliers.statement', $supplier->id)}}"
                    method="GET">
              ...
              <table class="table mb-0 table-borderless horizontal-table">
                <thead>
                  <tr class="userDatatable-header">
                    ...
                    <th>
                      <span class="userDatatable-title">{{ __('statement.supplier_debt') }}</span>
                    </th>
                    <th>
                      <span class="userDatatable-title">{{ __('statement.supplier_payment') }}</span>
                    </th>
                    <th>
                ...
                <tfoot>
                  ...
                  <tr>
                    <td colspan="5">
                      <div class="userDatatable-content">
                        <span>{{ __('statement.supplier_balance') }} :</span>
  ...
  function printTable() {
                                          ...
                                          // Function to handle downloading PDF
                                           function downloadPDF() {
                                            var options = {
                                              margin: 10,
                                              filename: '{{ $supplier->name .'-'. date('Y-m-d') }}.pdf',
